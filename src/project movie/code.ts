...
const searchButton = document.getElementById('search-bttn') ?? new HTMLElement();
const yearUp = document.getElementById('year-up') ?? new HTMLElement();
...
function searchInMovies(){
    ...
    filterByInputArray = movies.filter((movie) => movie[0].includes(searchField.value) || movie[1].includes(searchField.value) || movie[2].includes(searchField.value));
